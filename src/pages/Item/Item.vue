    ...
    <div class="wrap">
      ...
      <div class="m-cateNavVertWrap">
        <div class="j-cateNavWrap">
          <div class="j-cateNav">
            <div  class="item" v-for="(item1,index1) in nav.categoryL1Item" :key="index1" @click="getIndex(index1)" :class="{active:index === index1 }">{{item1.name}}</div>
          </div>
        </div>
      </div>
      <Item :nav="nav" :index="index"/>
    </div>
  </div>
</template>
